...
export default function Caracterizacion({ onContinuar, guardando, error }) {
  ...
  const handleEnviar = () => {
    const datosActor = { edad, colegio, grado, institucion, area, nivel_educativo: nivelEdu, organizacion, correo, participo_antes: participoAntes }
  ...
  const SUBPASOS = [
    ...
    <div key={0} className="space-y-6">
      ...
      <button onClick={irSiguiente} disabled={!nombre.trim()}
        ...
        style={nombre.trim() ? { background: '#67B93E', boxShadow: '0 4px 20px rgba(103,185,62,0.3)' } : { background: '#e5e7eb', color: '#9ca3af', cursor: 'not-allowed' }}>
        Continuar →
      </button>
    </div>,
    ...
    <div key={1} className="space-y-6">
      <div className="text-center mb-8">
        <span className="text-5xl block mb-3">📍</span>
        <h2 className="font-display text-3xl font-black text-gray-900 mb-2">Hola, <span style={{ color: '#67B93E' }}>{nombre}</span></h2>
      ...
      <Campo label="Corregimiento (opcional)">
        <input type="text" value={corregimiento} onChange={e => setCorregimiento(e.target.value)}
          placeholder="Nombre del corregimiento..." className={INPUT_CLASS}
          onFocus={e => e.target.style.borderColor = '#67B93E'}
          onBlur={e => e.target.style.borderColor = '#e5e7eb'} />
      </Campo>
      <Campo label="Vereda (opcional)">
        <input type="text" value={vereda} onChange={e => setVereda(e.target.value)}
          placeholder="Nombre de la vereda..." className={INPUT_CLASS}
          onFocus={e => e.target.style.borderColor = '#67B93E'}
          onBlur={e => e.target.style.borderColor = '#e5e7eb'} />
      </Campo>
      <button onClick={irSiguiente} disabled={!municipio}
        className="w-full py-4 rounded-2xl font-display font-bold text-lg text-white transition-all"
        style={municipio ? { background: '#67B93E', boxShadow: '0 4px 20px rgba(103,185,62,0.3)' } : { background: '#e5e7eb', color: '#9ca3af', cursor: 'not-allowed' }}>
        Continuar →
      </button>
    </div>,
    ...
    <div key={2} className="space-y-6">
      ...
      <button onClick={irSiguiente} disabled={!tipoActor}
        className="w-full py-4 rounded-2xl font-display font-bold text-lg text-white transition-all"
        style={tipoActor ? { background: actorInfo?.color || '#67B93E', boxShadow: `0 4px 20px ${actorInfo?.color || '#67B93E'}40` } : { background: '#e5e7eb', color: '#9ca3af', cursor: 'not-allowed' }}>
        Continuar →
      </button>
    </div>,
    ...
    <div key={3} className="space-y-5">
          ...
          <div className="flex flex-wrap gap-2">
            {GRADOS.map(g => (
              <button key={g} onClick={() => setGrado(g)}
                ...
                style={grado === g ? { background: '#67B93E', borderColor: '#3d7820', color: 'white' } : { background: 'white', borderColor: '#e5e7eb', color: '#374151' }}>
                {g}
              </button>
            ))}
          </div>
      ...
      {error && <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-red-600 font-body text-sm">⚠️ {error}</div>}

      <button onClick={handleEnviar} disabled={!edad || guardando}
        className="w-full py-4 rounded-2xl font-display font-bold text-lg text-white transition-all"
        style={edad && !guardando
